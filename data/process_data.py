...
def load_data(messages_filepath, categories_filepath):
    """Reads in and combines the messages and categories dataset."""
    df_messages = pd.read_csv(messages_filepath)
    df_categories = pd.read_csv(categories_filepath)

    df_merged = pd.merge(df_messages, df_categories, on='id', how='outer')
    
    return df_merged

def clean_data(df):
    """Splies categories into separate category columns."""
